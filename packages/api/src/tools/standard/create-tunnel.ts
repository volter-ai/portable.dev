...
// CREATE TUNNEL TOOL
// ============================================================================
// Creates or retrieves a Cloudflare Quick Tunnel for exposing a localhost port
// (any port). Used to make a local dev server publicly reachable.
// ============================================================================

/**
 * Create Tunnel Tool
 *
 * Get tunnel for a port. Use this after starting a dev server to make it publicly accessible.
 * Returns tunnel URL to use with mcp__playwright__browser_navigate.
 */
export const createTunnelTool = {
  name: 'create_tunnel',
  description: `Get a temporary HTTPS tunnel for a port (any port). Use this after starting a dev server to make it publicly accessible. Returns tunnel URL to use with mcp__playwright__browser_navigate.`,
  inputSchema: {
    type: 'object',
    properties: {
      port: {
        type: 'number',
        description: 'Port number of the dev server (e.g., 5173, 3000, 8000)',
      },
      name: {
        type: 'string',
        description:
  ...
  execute: async (input: any, context: ToolExecutionContext): Promise<ToolResult> => {
    ...
    // Create a Cloudflare Quick Tunnel for the requested port
    ...
    try {
      console.log(`[CreateTunnel] Creating Cloudflare Quick Tunnel for port ${port}...`);
      ...
      console.error('[CreateTunnel] create_tunnel error:', error);
      return {
        content: [
          {
            type: 'text',
